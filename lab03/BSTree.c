...
void BSTreeLevelOrder (BSTree t)
{
	...
	Queue Q = newQueue();
	for (QueueJoin(Q,t); QueueIsEmpty(Q) != 1; ) {
	    //output its number;
	    BSTree new_t = QueueLeave(Q);
	    showBSTreeNode(new_t);
	    
		//adding its children.
	    if (new_t->left != NULL) {
	        QueueJoin(Q,new_t->left);
	        
	    }
	    if (new_t->right != NULL) {
	        QueueJoin(Q,new_t->right);
...
{   
    int temp = 0;
    //if this node is null, we cannot grab its children.
	if (t == NULL) {
	
	    return 0;
	}
	//if its both children are null, adding one number,
	else if (t->left == NULL && t->right == NULL) {
	    
	    return 1;
	} else {
	    return temp + BSTreeNumLeaves (t->left) + BSTreeNumLeaves (t->right);
	}
}
