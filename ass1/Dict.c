...
////////////////////////////////////////////////////////////// 
//                   ALL My Help FUNCTION                   //
//////////////////////////////////////////////////////////////

//////////////////////////////////////
//   help functions for DictInsert  //
//////////////////////////////////////
...
// here using the AVL tree to build the whole struct.
Link doInsert(Link n, char *w, WFreq *pair) {
   if (n == NULL) {
      n = newNode(w);
      pair = n->data;
...
int checkorder (WFreq w1, WFreq w2) {
   int temp = 0;
   // if w1 has less freq
   // or with the same freq, w1 has a larger name.
   if ((w1.freq < w2.freq)
   || (w1.freq == w2.freq 
...
// sort the final version of wfs, using Insertion Sort method.
// from most freq to least
// for the same freq,
// sort from shortest word length to largest (using checkorder func).

// for the insert sort: we check the num1th number and previous number,
// swap this number into right position, 
// then, check the (num1 + 1)th one.
void SortPrintList(WFreq *wfs, int number) {  
   ...
   for (int num1 = 1; num1 < number; num1 ++) {
      ...
      while (num2 >= 0 && checkorder(wfs[num2], temp)) {
         wfs[num2 + 1] = wfs[num2];
         num2--;
      }
