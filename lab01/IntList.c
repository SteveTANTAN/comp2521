...
void IntListInsertInOrder (IntList L, int v) {
	...
	//if this number is the only one
    if (L->first == NULL ) {
        IntListInsert (L, v);
	
	
	//if this number is the smallest one.
	...
	//or it is in the middle
        ...
        struct IntListNode *new_node = newIntListNode (v);  
        struct IntListNode *tempnode = L->first;
        
        //serach for the midlle one,
        while(tempnode->next->data <= v) {
